...
class DEEP_GATGNN(torch.nn.Module):
        ...
        **kwargs
        ...
        if pre_fc_count > 0:
            ...
            for i in range(pre_fc_count):
                if i   == 0:
                    lin_N = torch.nn.Linear(num_features, dim1)
                    self.pre_lin_list_N.append(lin_N)
                    lin_E = torch.nn.Linear(num_edge_features, dim1)
    ...
    def forward(self, x, edge_index, edge_attr, glob_feat, batch):
        ...
        # print(out_x.shape)
        # exit()
        out_a       = self.global_att_LAYER(out_x,batch,glob_feat)
        out_x       = (out_x)*out_a            

        ##Post-GNN dense layers
        if self.pool_order == "early":
            if self.pool == "set2set":
                out_x = self.set2set(out_x, batch)
            else:
                out_x = getattr(torch_geometric.nn, self.pool)(out_x, batch)
