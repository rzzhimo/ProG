...
def process_data(data_path, processed_path, processing_args):
    ...
    for index in range(0, len(target_data)):
        ...
        if data_path == "./data/matbench_mp_is_metal":
            # 目标标签在每行数据的第二列
            labels = [row[1] for row in target_data]  # 根据数据结构调整
            # 使用LabelEncoder处理分类标签
            encoder = LabelEncoder()
            encoder.fit(labels)
            encoded_labels = encoder.transform(labels)
            y = torch.tensor([encoded_labels[index]], dtype=torch.long)
        else:
    ...
    # 在保存前清理数据, 删除不必要的数据, 以减小数据集的大小
    for data in data_list:
        if hasattr(data, 'edge_weight'):
            del data.edge_weight
        if hasattr(data, 'structure_id'):
            del data.structure_id
        if hasattr(data, 'u'):
            del data.u
        if hasattr(data, 'z'):
            del data.z
    ...
    if processing_args["dataset_type"] == "inmemory":
        ...
        # # 不要将data_list合并存储
        # torch.save(data_list, os.path.join(data_path, processed_path, "data_mb.pt"))
        # # 直接存储 data_list 为 .pkl 格式
        # with open(os.path.join(data_path, processed_path, "data_mb.pkl"), 'wb') as f:
...
if __name__ == "__main__":
    ...
    #Path to data files
    "data_path": "./data/matbench_phonons", #改成自己的数据集路径
    ...
    dataset = get_dataset(processing_args["data_path"], target_index, False, processing_args=processing_args)
